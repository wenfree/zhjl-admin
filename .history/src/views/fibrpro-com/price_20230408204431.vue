...
<el-row :gutter="20">
    <el-col :span="12">
        ...
        <el-form-item v-for="(pr, index) in form.product" :label="pr.model + '：'" :key="index">
            <el-input v-model="pr.acount" placeholder="請輸入數量" @change="ProductPrice()" ></el-input>

            <el-form-item label="规格">
                <el-radio v-model="pr.opration.size" v-for="item in sizeJson" :label="item.value" :key="item.value">{{ item.label }}</el-radio>
            </el-form-item>
