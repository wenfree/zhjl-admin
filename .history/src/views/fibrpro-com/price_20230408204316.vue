...
<el-row :gutter="20">
    <el-col :span="12">
        ...
        <el-form-item v-for="(item, index) in form.product" :label="item.model + '：'" :key="index">
            <el-input v-model="item.acount" placeholder="請輸入數量" @change="ProductPrice()" ></el-input>

            <el-form-item label="规格">
                <el-radio v-model="item.opration.size" v-for="item in sizeJson" :label="item.value" :key="item.value">{{ item.label }}</el-radio>
            </el-form-item>
            <el-form-item label="Thickness">
                <el-radio v-model="item.opration.Thickness" v-for="item in thickJson" :label="item.value" :key="item.value">{{ item.label }}</el-radio>
            </el-form-item>
            <el-form-item label="color">
                <el-input v-model="item.opration.color"></el-input>
            </el-form-item>
            <el-form-item label="slip">
                <el-radio v-model="item.opration.slip" v-for="item in slipJson" :label="item.value" :key="item.value">{{ item.label }}</el-radio>
            </el-form-item>
            <el-form-item label="max">
                <el-input v-model="item.opration.max"></el-input>
            </el-form-item>
            <el-form-item label="loading level">
                <el-radio v-model="item.opration.level" v-for="item in levelJson" :label="item.value" :key="item.value">{{ item.label }}</el-radio>
            </el-form-item>
        </el-form-item>
...
export default {
    ...
    methods: {
        ...
        addProducts(products){
            ...
            this.form.product.push({model:products, acount:'', opration:{  size:'', Thickness:'', color:'', slip:'',max:'',level:'',}})
        },
        handleClose(index) {
