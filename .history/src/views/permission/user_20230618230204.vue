            ...
            <template v-for="(item, index) in formJson">
              ...
              <el-form-item v-else-if="item.type == 'selects'" :key="item.prop" :label="item.label" :prop="item.prop">
                <el-select
                  ...
                  placeholder="请选择"
                >
                  <el-option v-for="item_ in item.options" :key="item_.role" :label="item_.role_name" :value="item_.role" />
                </el-select>
              </el-form-item>
              ...
              <el-form-item v-else-if="item.type == 'password'" :key="item.label" :label="item.label" :prop="item.prop">
                <el-input v-model="form[item.prop]" :disabled="item.disable" show-password/>
              </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      formJson: [
        { label: '登录代码', prop: 'username', disable: true },
        { label: '用户名称', prop: 'fullName' },
        { label: '密码', prop: 'password', type: 'password' },
        { label: '确认密码', prop: 'password_confirm', type: 'password' },
        // { label: '昵称', prop: 'nickname' },
        {
          label: '用户类型', prop: 'category', type: 'select', options: [
            { label: '内部员工', value: '内部员工' },
            { label: '外部员工', value: '外部员工' },
          ]
        },
        { label: '企业名称', prop: 'unit', type: 'autocomplete' },
        {
          label: '用户校色', prop: 'roles', type: 'selects', options: [
  ...
  methods: {
    ...
    getListRoles() {
      ...
      const postData = {
        page:1,
        size:99,
        table: 'phalapi_zhjl_role',
        where:''
      }

      fetchList(postData).then(response => {
        console.log('取参数roles', response.data)
        this.formJson[6].options = response.data.data
      })
    },
    getList() {
      this.listLoading = true
