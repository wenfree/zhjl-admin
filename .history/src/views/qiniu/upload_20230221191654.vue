<template>
  <el-upload :file-list="fileList" :data="dataObj" :multiple="true" :before-upload="beforeUpload" :on-error="uploadError"
    accept="image/jpeg, image/jpg, image/png" action="https://qiniu.region.z2" :on-success="handleSuccess" drag>
    ...
    beforeUpload(file) {
      ...
      return new Promise((resolve, reject) => {
        getToken().then(response => {
          ...
          _self._data.dataObj.key = `upload_pic_${new Date().getTime()}_${file.name}`
