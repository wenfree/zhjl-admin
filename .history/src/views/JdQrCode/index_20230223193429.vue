<template>
  <div v-if="is_login">
    ...
    <el-dialog title="二维码展示" :visible.sync="dialogVisible" width="95%">
      ...
      <div class="showQrecode">
        ...
        <div class="canvas_box"><canvas ref="QrCode" id="QrCode"></canvas></div>
      </div>
  ...
  <div v-else>
    <div class="login-container">
      <div class="banner_img" style="display: flex;justify-content: center">
        <el-image style="height: 100px;margin-top: 15%;" src="https://img.wenfree.cn/logo/logo-yunkong.png"
      ...
      <el-form ref="form" :model="form" :rules="Rules" label-width="200px" label-position="top">
        ...
        <template v-for="item in formJosn">
          ...
          <template v-if="item.type == 'text'">
            <el-form-item label="" :key="item.key">
              <p style="font-size:16px;margin-top: 0px;line-height: 14px;text-align: center;">{{ item.label }}</p>
            </el-form-item>
          </template>
          <template v-if="item.type == 'title'">
            <el-form-item label="" :key="item.key">
...
export default {
  ...
  data() {
    return {
      ...
      is_login: false,
      formJosn: [
        { label: '非常简单，就是好用', type: 'text', placeholder: '', key: 'disc' },
      ...
      formJosn2: [
        ...
        { label: '手机号', type: 'input', placeholder: '请输入手机号', key: 'phone' },
        { label: '身份证号', type: 'input', placeholder: '请输入身份证号', key: 'IdNum' },
        { label: '详细地址', type: 'input', placeholder: '请输入详细地址', key: 'Address' },
      ],
      form: {
        Name: '',
        phone: '',
