<template>
  <div class="app-container">
    ...
    <el-drawer :title="create.title" :visible.sync="create.drawer" direction="ltr" size="95%">
      <el-main>
        <el-form ref="form" :model="form" label-width="100px" class="form_createNew">
          <template v-for="(item, index) in formJson">
            <el-form-item :key="index" :label="item.label + ':'" :prop="item.prop" :class="item.prop">
              ...
              <template v-else-if="item.type == 'textarea'">
                <el-input v-model="form[item.prop]" type="textarea" :rows="3" :placeholder="item.placeholder" :disabled="item.disable" />
              </template>
              <template v-else>
        ...
        <div class="yiqin">
          ...
          >
            ...
            <el-table-column v-for="(item, index) in Yiqi_column" :key="index" :prop="item.prop" :label="item.label" />
          </el-table>
        </div>

        <div class="filter-container" style="margin-top: 20px;align-items: flex-end;">
          <el-button
            :loading="listLoading"
            type="success"
            icon="el-icon-orange"
  ...
  data() {
    return {
      ...
      formJson: [
        ...
        { prop: 'instrument_name', label: '预约日期', type: 'time',style: 'width:100%;' },
        { prop: 'model_specifications', label: '仪器数量' },
        { prop: 'manufacturer', label: '服务方式', type:'selects', options: ['送检', '现场'] },
        { prop: 'factory_number', label: '证书单位', type:'selects', options: ['泽恒计量有限公司', '中国计量有限公司'] },
        { prop: 'management_number', label: '证书地址' },
        { prop: 'installation_location', label: '证书类型', type:'selects', options: ['电子证书', '纸质证书', '纸质证书+电子证书'] },      
  ...
  methods: {
    ...
    getListYiqi() {
      ...
      fetchList(postData).then(response => {
        console.log('取仪器', response.data)
        this.yiqiList = response.data.data
