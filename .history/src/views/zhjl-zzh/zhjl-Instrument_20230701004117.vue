<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          <el-button size="mini" icon="detel" @click="history(scope.row)">证书</el-button>
          <el-button size="mini" icon="detel" @click="yuyue(scope.row.id)">预约</el-button>
    ...
    >
      <el-main>
        <el-form :model="historyList" label-width="100px">
          <el-form-item label="仪器名称：" style="margin-bottom: 4px; width: 340px;">
            <el-input v-model="historyList.instrument_name" ></el-input>
          </el-form-item>
          <el-form-item label="制造厂家：" style="margin-bottom: 4px; width: 340px;">
            <el-input v-model="historyList.manufacturer"></el-input>
          </el-form-item>
          <el-form-item label="型号规格：" style="margin-bottom: 4px; width: 340px;">
            <el-input v-model="historyList.model_specifications"></el-input>
          </el-form-item>
          <el-form-item label="出厂编号：" style="margin-bottom: 4px; width: 340px;">
            <el-input v-model="historyList.factory_number"></el-input>
          </el-form-item>
          <el-form-item label="管理编号：" style="margin-bottom: 16px; width: 340px;">
            <el-input v-model="historyList.management_number"></el-input>
          </el-form-item>
        </el-form>
        ...
        >
          ...
          <el-table-column label="操作" width="360">
            <template slot-scope="scope">
              <el-button size="mini" icon="detel" @click="todoZS('下载证书')">下载证书</el-button>
              <el-button size="mini" icon="detel" @click="todoZS('修改证书')">修改证书</el-button>
              <el-button size="mini" icon="el-icon-edit" @click="todoZS('确认证书')">确认证书</el-button>
            </template>
          </el-table-column>
        </el-table>

        <el-button v-waves style="margin-top: 15px;" class="filter-item" type="success" icon="el-icon-plus" @click="upload">
          证书上传
        </el-button>

        <el-button v-waves style="margin-top: 15px;" class="filter-item" type="info" icon="el-icon-plus" @click="drawer = false">
          返回
        </el-button>
    ...
    <el-dialog
      :title="ZS_todo.title"
      :visible.sync="ZS_todo.Visible"
      width="30%"
      center>
      <span>成功提交证书修改</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="ZS_todo.Visible = false">取 消</el-button>
        <el-button type="primary" @click="ZS_todo.Visible = false">确 定</el-button>
      </span>
    </el-dialog>

    <el-drawer :title="certificate.title" :visible.sync="certificate.drawer" direction="ltr" size="80%">
      <el-main>
        
        <el-form ref="form" :model="form" label-width="150px" class="">
          <template v-for="(item, index) in formJson2">
            <el-form-item :key="index" :label="item.label + ':'" :prop="item.prop" :class="item.prop" style="width: 400px;">
              <template v-if="item.type == 'Checkbox'">
                <el-checkbox-group v-model="form[item.prop]">
                  <el-checkbox v-for="check in item.opration" :key="check" :label="check">{{ check }}</el-checkbox>
                </el-checkbox-group>
          ...
          <div class="filter-container">
            ...
            <el-button
              ...
              class="filter-item"
              size="mini"
              @click="certificate.drawer = false"
            >
              返回
            </el-button>
          </div>
...
import { fetchList, deleteById, updateArticle, UpdateById } from '@/api/zhjl.js'
...
export default {
  ...
  data() {
    return {
      ...
      formJson2: [
        { prop: 'instrument_name', label: '溯源机构' },
        { prop: 'model_specifications', label: '校准日期' , type:'time'},
        { prop: 'manufacturer', label: '证书编号' },
      ],
      historyArr: [
        {
          校准日期: '20220415',
  ...
  methods: {
    ...
    todoZS(type){
      console.log('todoZS', type)
      this.ZS_todo.title = type
      this.ZS_todo.Visible = true
    },
    upload(){
      this.certificate.drawer = true
      this.certificate.title = '上传证书记录'
      this.certificate.button = '上传'
    },
  }
}
</script>
